<script lang="ts">
    ...
    const items: StepMenuItem[] = [{ icon: 'pi pi-user' }, { icon: 'pi pi-calendar' }, { icon: 'pi pi-check' }]
    const codeItems = `
    const items: StepMenuIem[] = [{ icon: 'pi pi-user' }, { icon: 'pi pi-calendar' }, { icon: 'pi pi-check' }]
`
    const codeSnippet = `
{#snippet itemSnippet({ item, index, stepOptions: { active, onClick } }: StepMenuIemSnippetProps)}
    <button
...
<DocSectionText {docSection}>
    <p>
        Steps offers "all items" customization with <i>itemSnippet</i> property that receives the item instance and returns an element
    </p>
</DocSectionText>
<div class="card">
    <Steps model={items} readOnly={false} {itemSnippet} class="m-2 pt-4" />
</div>
<DocSectionCode {code} />

{#snippet itemSnippet({ item, index, options: { active, onClick } }: StepMenuItemSnippetProps)}
    <button
        class="inline-flex align-items-center justify-content-center align-items-center border-circle border-primary border-1 h-3rem w-3rem z-1 cursor-pointer"
        style={[
            `background-color: ${active ? 'var(--primary-color)' : 'var(--surface-b)'}`,

...
<DocSectionText docSection={{ id: 'accessibility', label: 'Accessibility' }}>
    <h3>Screen Reader</h3>
    <p>
        Message components use <i>alert</i> role that implicitly defines <i>aria-live</i> as "assertive" and <i>aria-atomic</i> as
        "true". Since any attribute is passed to the root element, attributes like <i>aria-labelledby</i> and{' '}
    ...
    <DocSimpleTable
        ...
        rows={[
            ['<i>enter</i>', 'Closes the message.'],
            ['<i>space</i>', 'Closes the message.']
        ]}
    />
</DocSectionText>

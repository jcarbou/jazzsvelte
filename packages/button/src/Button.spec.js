...
describe('Button', () => { 
    ...
    containerTest('when button is clicked ensure onClick is fired', Button, {label:'test'}, async (buttonEl, button) => {
        ...
        if (buttonEl && button) {
            ...
            // Assert
            expect(buttonEl.attributes.disabled).not.toBeDefined()
            expect(clickOn).toHaveBeenCalledTimes(1)
        }
    })

    containerTest('when button is disabled the click event should not fire', Button, {label:'test', disabled:true}, async (buttonEl, button) => {
        expect(buttonEl).not.toBeNull()
        expect(button).not.toBeNull()
        ...
        if (buttonEl && button) {            
            ...
            // Assert
            expect(buttonEl.attributes.disabled).toBeDefined()
